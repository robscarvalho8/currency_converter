...
class CurrencyConvertService():
    ...
    def get_convert_from(self):
        self.convert_from = self.db.query(Currency)\
            .filter(Currency.abbreviated == self.coin_from)\
            .first()
        if not self.convert_from:
            return self.error.append({
                "detail": f"{self.coin_from} is not a valid coin."
            })

    def get_convert_to(self):
        self.convert_to = self.db.query(Currency)\
            .filter(Currency.abbreviated == self.coin_to)\
            .first()
        if not self.convert_to:
            return self.error.append({
                "detail": f"{self.coin_to} is not a valid coin."
            })

    def convert_amount(self):
        if not self.amount_from:
            return self.error.append({"detail": "Amount is required."})
        else:
            self.amount_to = round(
                (self.amount_from / self.convert_from.dolar_quotation)
                * self.convert_to.dolar_quotation, 8)
